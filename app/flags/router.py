...
@router.patch("/flag-environments/{flag_env_id}", response_model=FlagResponse)
async def update_flag_environment(
    flag_env_id: UUID,
    body: FlagEnvironmentUpdate,
    db: AsyncSession = Depends(get_db),
    user: User | None = Depends(get_current_user),
):
    return await flag_service.update_flag_environment(
        db, flag_env_id, body, changed_by=_actor(user)
    )


@router.post("/flag-environments/{flag_env_id}/toggle", response_model=FlagResponse)
async def toggle_flag_environment(
    flag_env_id: UUID,
    db: AsyncSession = Depends(get_db),
    user: User | None = Depends(get_current_user),
):
    return await flag_service.toggle_flag_environment(
        db, flag_env_id, changed_by=_actor(user)
    )
