...
async def _get_env(client, project_id, env_name="development"):
    """Get environment dict by name for a project."""
    resp = await client.get(f"/projects/{project_id}/environments")
    envs = resp.json()
    return next(e for e in envs if e["name"] == env_name)


async def _publish_and_capture(db_session, pid, environment_id, sdk_key):
    ...
    with (
        ...
        await publish_flags(db_session, UUID(pid), UUID(environment_id), sdk_key)
...
async def test_publish_generates_correct_json_structure(client, db_session):
    ...
    env = await _get_env(client, pid, "development")
    data = await _publish_and_capture(db_session, pid, env["id"], env["sdk_key"])
